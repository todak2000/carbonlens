...
function resultsSection(result: SimulationResult): PermitSection {
  return {
    ...
    lines: [
      ...
      'TRAPPING MECHANISMS',
      sep,
      l('Mobile Plume', `${result.mobilePlume.toFixed(2)} Mt`),
      l('Residual (Ganglia)', `${result.residualTrapping.toFixed(2)} Mt`),
      l('Dissolved (Solubility)', `${result.solubilityTrapping.toFixed(2)} Mt`),
      sep,
    ],
  }
}
